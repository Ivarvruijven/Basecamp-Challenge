...
def black_jack():
    ...
    soorten = ["Harten", "Schoppen", "Ruiten", "Klaver"]
    ...
    if hand_waarde(hand) < 21:
        keuze = int(input("Maak je keuze (nummer):\n1. Hit\n2. Stand\n"))
    ...
    if hand_waarde(hand) < 21:
        while hand_waarde(computer) < 17:
            if hand_waarde(computer) < 21:
                ...
                sleep(1)
                print()
    if (hand_waarde(computer) > 21 and hand_waarde(hand) > 21) or (hand_waarde(computer) == hand_waarde(hand)):
        print("Push!")
        print(f"Waarde user: {hand_waarde(hand)}")
        print(f"Waarde computer: {hand_waarde(computer)}")
        userbalance+=inzet
